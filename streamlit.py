        ...
        if st.button("Send", key="chat_submit"):
            if not question.strip():
                ...
                if result and "answer" in result:
                    ...
                    if "faqs" in result and result["faqs"]:
                        st.markdown("---")
                        if "faq_intro" in result:
                            st.markdown(f"#### {result['faq_intro']}")
                        else:
                            st.markdown("#### Here are some related FAQs:")
                        
                        for i, faq in enumerate(result["faqs"], 1):
                            with st.expander(f"FAQ {i}: {faq['question'][:60]}{'...' if len(faq['question']) > 60 else ''}"):
